...
class QuotationService {
    ...
    async create(dto, currentUser) {
        if (!dto.leadId && !dto.customerId) {
            throw new shared_types_1.AppError("Either leadId or customerId must be provided.", 400);
        }
        // Generate quotation number (e.g., QT-YYYYMMDD-XXXX)
        const count = await prisma_1.default.quotation.count();
        ...
        return prisma_1.default.quotation.create({
            data: {
    ...
    async findById(id) {
        const quote = await prisma_1.default.quotation.findUnique({
            where: { id },
            include: {
        ...
        if (!quote)
            throw new shared_types_1.AppError("Quotation not found.", 404);
        return quote;
    }
    // ── Generate PDF ────────────────────────────
    async generatePdf(id) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (client?.company)
                    doc.text(client.company, 300);
                if (client?.email)
                    doc.text(client.email, 300);
                if (client?.phone)
                    doc.text(client.phone, 300);
                doc.moveDown(3);
                // Table Header
